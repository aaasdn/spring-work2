...
@RestController // 이것을 작성하면 메서드마다 리스폰스바디를 붙힌 효과를 받는다 이것을 작성하면 뷰로 가는게아니라 
//클라이언트(data)로 던져짐 현재 snsList의 리턴타입이 void라서 리턴값을 받지 않기때문에 화면에 뜨지않는다.
//restcontroller에서 controller로 바꾼후 메서드(비동기)마다 ResponseBody(데이터반환)를 붙히면됨. 아니면!?
//modelandView :응답 받는 객체 / 페이지전환할때 가끔쓰임 => 쓰는이유 레스트컨트롤러를 써도 모델앤드뷰를 써서 리턴해도
//클라이언트로 던져지지않고 뷰리졸버에게 그대로 전달된다.
...
//		System.out.println("fileName: " + fileName);
	...
	public ResponseEntity<?> download(@PathVariable String fileLoca,
		...
		HttpHeaders header = new HttpHeaders(); //브라우저에 단순히 노출할때는 필요없음
		//하지만 다운로드를 제공해야할때는 헤더에다가 제공된 명칭을 줘야지 다운로드로 제공됨
