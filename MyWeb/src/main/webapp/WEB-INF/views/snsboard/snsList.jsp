...
<body>
	...
	<script>
		...
		function regist() {
			...
			const userId = '${sessionScope.login}'; //그냥 login만 넣어도됨
			if(userId === '') { //세션 데이터가 없다? -> 로그인x
			...
			*/
			...
			//한 화면에 파일 업로드 버튼이 여러 개일 경우 요소의
			//인덱스를 지목해서 가져올 수 있음.
            //우리는 파일 첨부 버튼이 하나고,
			//id를 지목해서 가져오기 때문에 인덱스를 쓸 필요는 없습니다.
            ...
            //files -> FileList에 여러 파일의 정보가 들어오게 됩니다. files: 여러개 다들어옴
			...
			fetch('${pageContext.request.contextPath}/snsboard', { //snsboard가 post방식으로 전송됨.
				method: 'post',
				body: formData
			})
			//응답 데이터는 text로 전달됩니다. (uploadSuccess)
			//file input 내용을 비워 주시고, 글 영역 비워 주시고,
			//class 이름이 fileDiv(미리보기)영역을 감춰주세요. -> display 속성을 none으로.
			.then(res => res.text())
			.then(data => {
				console.log(data);
		...
		function getList(page, reset) {
			...
			fetch('${pageContext.request.contextPath}/snsboard/' + page) //page이어붙히기
				.then(res => res.json())
				.then(list => {
					console.log(list);
					...
					for(board of list) {
						...
						`<div class="title-inner">
                            ...
                            <div class="title">
                                ...
                                <a id="download" href="${pageContext.request.contextPath}/snsboard/download/` + board.fileLoca + `/` + board.fileName + `">이미지 다운로드</a>                            </div>
                        </div>
                        <div class="content-inner">
                            <!--내용영역-->
                            <p>` + board.content + `</p>
		...
		$contentDiv.addEventListener('click', e => {
			...
			.then(res => res.json()) //객체라 json으로 와야함
			.then(data => {
				console.log(data);

			$(document).ready(function() {
				$("#snscontent").click(function() {
					$('#id').modal('show');
				});
