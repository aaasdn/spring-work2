...
//   private FreeBoardController controller; -> 이 컨트롤러 하나만 쓸 때.
   ...
   public void setup() {
      ...
      //테스트할 컨트롤러를 수동으로 주입. 하나의 컨트롤러 자체만 테스트를 진핼할 때는 이렇게 씁니다.
//      this.mockMvc = MockMvcBuilders.standaloneSetup(controller).build();
   ...
   void testContent() throws Exception { //예외처리 13번
      // /freeboard/content -> get
	  // bno, title, writer, content, updateDate == null ? regDate, updateDate(수정됨)
      
     ModelAndView mv = mockMvc.perform( // 모델뷰 객체 선언 12번
    		 					MockMvcRequestBuilders.get("/freeboard/content")
    		  						.param("bno", "1") //1번
    		  					).andReturn().getModelAndView(); //11번
     
     assertEquals("freeboard/freeDetail", mv.getViewName()); //14번 
     FreeDetailResponseDTO dto = (FreeDetailResponseDTO) mv.getModelMap().get("article"); 
     //15번 아티클이라는 이름으로 dto에 담고, 형변환
     System.out.println(dto);
     assertEquals(dto.getBno(), 1); // 16번 
   ...
   void testModify() throws Exception {
      String bno = "3";
	  String viewName = mockMvc.perform( //1번. 8번.->변수 선언
			   MockMvcRequestBuilders.post("/freeboard/modify")
			   						 .param("bno", bno)
			   						 .param("title", "제목수정")
			   						 .param("content", "내용수정")
			   			).andReturn().getModelAndView().getViewName(); //7번.
	   assertEquals(viewName, "redirect:/freeboard/content?bno=" + bno); // 9번.
   }
   ...
   void testDelete() throws Exception {
       assertEquals("redirect:/freeboard/freeList", // post방식이라 리다이렉트로 주소값설정.
    		   mockMvc.perform(MockMvcRequestBuilders.post("/freeboard/delete")
    				   .param("bno", "3")
    				   ).andReturn().getModelAndView().getViewName()
    		   ); //1번.. 컨트롤러부터 리턴의 모델엔뷰의 뷰네임을 결과값을 얻어낸다 
       	// 이것은 redirect:/freeboard/freeList 와 같다는것을 단언한다. assertEquals를 쓰는 이유다.
   }
